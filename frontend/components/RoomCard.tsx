...
import { Users, DoorOpen, Layers, Sparkles, RefreshCw } from "lucide-react";
...
export default function RoomCard({ room, onOccupancyChange }: RoomCardProps) {
  ...
  return (
    <div className="glass-card rounded-2xl p-5 border border-gray-800 flex flex-col justify-between hover:border-blue-500/30">
      <div>
        {/* Header */}
        ...
        <div className="flex flex-wrap items-center gap-2 text-xs text-gray-400 mt-3">
          <span className="px-2.5 py-1 rounded-lg bg-gray-800/60 border border-gray-700/50 capitalize font-medium text-gray-300">
            {room.room_type}
          </span>
          <span className="flex items-center gap-1 px-2.5 py-1 rounded-lg bg-gray-800/60 border border-gray-700/50">
            <Users className="w-3.5 h-3.5 text-blue-400" /> Capacity: {room.capacity}
          </span>
        ...
        <div className="mt-4 p-3 rounded-xl bg-slate-900/60 border border-gray-800">
          <div className="flex justify-between items-center text-xs mb-1.5 font-medium">
            <span className="text-gray-400">Live Occupancy Simulator:</span>
            <span className="text-white font-bold">{currOcc} / {room.capacity} people</span>
          </div>
          <input
            type="range"
            min="0"
        ...
        {/* AI Forecast Collapsible Box */}
        {showPrediction && prediction && (
          <div className="mt-3 p-3.5 rounded-xl bg-indigo-950/40 border border-indigo-500/30 text-xs animate-fadeIn">
      ...
      <div className="mt-4 pt-3 border-t border-gray-800/80 flex items-center justify-between">
        <button
          ...
          className="text-xs font-semibold text-indigo-400 hover:text-indigo-300 flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-indigo-500/10 border border-indigo-500/20 hover:bg-indigo-500/20 transition-all"
        >
          <Sparkles className="w-3.5 h-3.5" />
          {showPrediction ? "Hide AI Prediction" : "Predict 30m Crowd"}
        </button>
      </div>
    </div>
  );
